...
int main()
{
    ...
    // bool clicked_on_wall = false, monster_clicked_on_wall = false;
    // bool err_flag = false, monster_err_flag = false;
    ...
    ofstream csv;
    csv.open("DecisionTreeLearning.csv");
    csv << "Sprite Room,Monster Room,Same Room,Neighbor Room,Distance,State,Eats\n";
    ...
    while (window.isOpen())
    {
        ...
        if(abs(monster.get_position().x - sprite.get_position().x ) < 10 && abs(monster.get_position().y - sprite.get_position().y ) < 10)
        {
            ...
            if(current_room == monster_current_room)
            {
                ...
                // Dumping to CSV
                csv<<current_room<<",";
                csv<<monster_current_room<<",";
                csv<<(monster_current_room == current_room)<<",";

                bool neighboring_room = false;
                for(auto adj_room: rooms[current_room].adjacent_rooms)
                    if (monster_current_room == adj_room) neighboring_room = true;  

                csv<<neighboring_room<<",";
                csv<<sqrt(pow(sprite.get_position().x - monster.get_position().x, 2) + pow(sprite.get_position().y - monster.get_position().y, 2))<<",";
                csv<<"Reset"<<",";
                csv<<reset_flag<<"\n";
            }
        ...
        if( sprite.get_position().x - 3 <= wander_point.x && wander_point.x <= sprite.get_position().x + 3 && sprite.get_position().y - 3 <= wander_point.y && wander_point.y <= sprite.get_position().y + 3)
        {
                ...
                if(clock.getElapsedTime().asSeconds() - start_time > 3)
                {
                ...
                {
                    if(sprite.get_position().x - rooms[current_room].bottom_left.x <= 5 || sprite.get_position().y - rooms[current_room].top_left.y <= 5 || rooms[current_room].bottom_right.x - sprite.get_position().x <= 5 || rooms[current_room].bottom_left.y -  sprite.get_position().y <= 5 )
                    {
                ...
                // if(isnan(wander_point.x)){
                //     cout<<"Wander point is isnan"<<endl;
                // }
                // if(isnan(wander_point.y)){
                //     cout<<"Wander point is isnan"<<endl;
                // }
        ...
        {
            sprite.set_target(mouse_final_float);
            if(isnan(mouse_final_float.x) || isnan(mouse_final_float.y)){
                // cout<<"Mouse final float is nan"<<endl;
            }
        }   
        ...
        {
                // Logic to dump each parameter into a csv file on a per frame basis
                csv<<current_room<<",";
                csv<<monster_current_room<<",";
                csv<<(monster_current_room == current_room)<<",";
                ...
                if(current_room == monster_current_room)
                {
                    // cout<<current_room<<monster_current_room<<endl;
                    // Seek
                ...
                {
                    if(monster_clock.getElapsedTime().asSeconds() - monster_start_time > 3)
                    {
                        // Scripted path/Change room
                        int next_room = scripted_path.back();
                        scripted_path.pop_back();
                        scripted_path.insert(scripted_path.begin(), next_room);
                        // while(rand_room == monster_current_room) rand_room = rand() % 7;
                ...
                {   
                    ...
                    {
                        for (int vertex : g.dijkstra(monster_current_room, monster_clicked_room) )
                        {
                            // cout<<vertex<<endl;
            ...
            for(int i = monster_path.size() - 1;i >=0;i--)
            {
                for(auto entrance: rooms[monster_path[i]].room_entry)
                {
                    for(auto entrances: rooms[monster_current_room].room_entry)
                    {
                        if(entrances == entrance) 
                        {
                            // cout<<rooms[monster_path[i]].room_id<<" "<<rooms[monster_current_room].room_id<<endl;
