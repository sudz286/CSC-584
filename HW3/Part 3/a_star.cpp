...
class Graph
{
    public:
        ...
        vector<int> path_finder(int first, int final, int manhattan)
        {
            ...
            while (!nodes.empty())
            {
                ...
                {
                    int alt;
                    // Calculate Manhattan Distance as heuristic else Euclidean distance squared
                    if(manhattan) alt = weights[closest] + neighbor.second + (abs(heuristic[closest][0]-heuristic[final][0])*54) + (abs(heuristic[closest][1]-heuristic[final][1])*69 );
                    else alt = weights[closest] + neighbor.second + ( (abs(heuristic[closest][0]-heuristic[final][0])*54) * (abs(heuristic[closest][0]-heuristic[final][0])*54) +  abs(heuristic[closest][1]-heuristic[final][1])*69 * (abs(heuristic[closest][1]-heuristic[final][1])*69) );
...
int main(int argc, char *argv[])
{
    ...
    if(argc < 5){ cout<<"CLA 1-> Large or Small Graph, 2->Source Node, 3->Dest Node 4->Manhattan or Euclidean Squared"<<endl; exit (1);}

    int large = stoi(argv[1]);
    int source = stoi(argv[2]), dest = stoi(argv[3]);
    int manhattan = stoi(argv[4]);
    int max_vertices = 29;
    
    if(large != 1 && large != 0){ cout<<"Enter either 0 or 1 and second CLA"<<endl; exit (1);}
    if(manhattan != 1 && manhattan != 0) { cout<<"Enter either 0 or 1 for heuristic choice"<<endl; exit (1);}
    if(large)
    {
    ...
    vector <int> answer = g.path_finder(source, dest, manhattan);
